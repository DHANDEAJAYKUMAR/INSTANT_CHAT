...
const ProfilePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedImg) {
      const updated = await updateProfile({ fullName: name, bio });
      if (updated) setAuthUser((prev) => ({ ...prev, fullName: name, bio }));
      navigate('/');
      return;
    }
  ...
  return (
    <div className='relative min-h-screen flex items-center justify-center bg-cover bg-no-repeat'>
      <button
        onClick={() => navigate(-1)}
        className='absolute top-5 right-5 bg-violet-600 text-white px-4 py-2 rounded-full shadow-md hover:bg-violet-700 transition'
      >
        ⬅ Back
      </button>
